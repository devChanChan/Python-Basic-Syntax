...
def is_validated_english_sentence(user_input):
    ...
    input_replace_space = input_strip.replace(" ", "")
    input_replace_dot = input_replace_space.replace(".", "")
    intput_replace_comma = input_replace_dot.replace(",", "")
    intput_replace_excla = intput_replace_comma.replace("!", "")
    intput_replace_quest = intput_replace_excla.replace("?", "")

    if user_input == "":
        result = False
    elif intput_replace_quest == "":
        result = False
    else:
...
def is_validated_morse_code(user_input):
    ...
    result = True
    morse_code = get_morse_code_dict()
    input_repl = user_input.replace("-", "").replace(".", "").replace(" ", "")
...
def get_cleaned_english_sentence(raw_english_sentence):
    ...
    # ===Modify codes below=============
    # 조건에 따라 변환되어야 할 결과를 result 변수에 할당 또는 필요에 따라 자유로운 수정
    input_del_sign = raw_english_sentence.replace(".", "").replace(",", "")
    input_del_sign = input_del_sign.replace("!", "").replace("?", "")
    input_strip = input_del_sign.strip()
...
def decoding_sentence(morse_sentence):
    ...
    # 조건에 따라 변환되어야 할 결과를 result 변수에 할당 또는 필요에 따라 자유로운 수정
    result = ""
    input_strip = morse_sentence.strip()
    input_first = input_strip.split("  ")
    for i in input_first:
        input_second = i.split(" ")
        for j in input_second:
            result += decoding_character(j)
        result += " "
    result = result.strip()
    return result
    # ==================================
...
def encoding_sentence(english_sentence):
    ...
    upper_case = cleaned_english_sentence.upper()
    input_first = upper_case.split()
    for i in input_first:
        for j in i:
